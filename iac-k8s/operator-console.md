...
Each screen below lists its **purpose**, **layout** (the Bootstrap pieces used), **features**, and the **backend action** it triggers (endpoints defined in [design.md](design.md)). Every change-making action follows the same rule: it opens a reviewed pull request — nothing is applied without approval (requirement R8). To see the styled, navigable screens, open [`console-mockup.html`](console-mockup.html) in a browser.
