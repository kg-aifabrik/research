...
## Decision: Terraform execution backend = GitHub Actions (D11)

**Decided** — **GitHub Actions** is the Terraform execution backend, with **Workload Identity Federation** (keyless) for GCP auth and **Environments + required reviewers** as the apply approval gate. The console drives it through the Actions API: `workflow_dispatch` to trigger plan/apply, poll run status, and pull the plan artifact to render the diff. For production, run **self-hosted runners on the FOP** so the WIF token exchange and apply execute in-environment; the POC may start on GitHub-hosted runners for speed. Atlantis and HCP Terraform/TFE were considered and rejected (an extra service to operate / an external SaaS dependency + cost). See the [implementation plan](implementation-plan.md).
