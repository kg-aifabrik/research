...
- **Architecture is settled** ([shared-instance-architecture.md](shared-instance-architecture.md)): official Helm chart (production-ready since April 2026) into the existing GKE cluster, Cloud SQL PostgreSQL in regional HA, no Elasticsearch. Two decisions need care because they're hard to undo: the history shard count (512 — permanent once set) and security (Temporal ships with no authentication at all).
